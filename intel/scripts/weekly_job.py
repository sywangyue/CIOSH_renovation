...
from services.reporter import build_weekly_html
...
def main() -> None:
    ...
    # 幂等：当周已跑过则退出
    if conn.execute(
        "SELECT 1 FROM reports WHERE report_date=? AND report_type='weekly'", (today,)
    ).fetchone():
        print(f"本周周报已存在（{today}），跳过。")
        conn.close()
        return
    ...
    # 写回 keyword_db.json
    with open(cfg.KEYWORD_DB_PATH, "w", encoding="utf-8") as f:
    ...
    rows = conn.execute("""
        ...
        LIMIT 30
    """).fetchall()
    high_items = [dict(r) for r in rows]

    html = build_weekly_html(high_items, stats, added, retired, week_label)
    print(f"Step5: 周报 HTML 生成（高优先级情报 {len(high_items)} 条）")

    # 保存到 reports 表
    conn.execute("""
        INSERT INTO reports (report_date, report_type, title_zh, html_body, item_count)
        VALUES (?, 'weekly', ?, ?, ?)
    """, (today, f"CIOSH情报周报 {week_label}", html, len(high_items)))
    conn.commit()
    ...
    # Step 6: 发送邮件
    subject = f"[CIOSH情报] {week_label} 周报 · 高优先级 {len(high_items)} 条"
    if added:

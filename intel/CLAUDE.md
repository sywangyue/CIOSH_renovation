...
**三层架构：**
- **采集层**：Tavily（国际）+ 百度新闻 + 知乎 + B站（国内），全量关键词搜索，无 URL seed 信源，无任何手动录入通道
- **处理层**：三层漏斗（URL 去重 → 标题评分 → DeepSeek 分析），每日 AI 分析硬上限 20 条
...
### Token 纪律
- `batch_analyze()` 每日硬上限 `MAX_LAYER3_PER_DAY`（默认 20），任何情况不得绕过
...
*CIOSH Intel Radar · CLAUDE.md v1.1 · 2026-06-04*
